...
// ════════════════════════════════════════════════════════════════════════
// SALES STATS — what sold, and how many times (this week / this month)
// ════════════════════════════════════════════════════════════════════════
...
function renderSalesTable(title, subtitle, statsResult) {
    ...
    sub.style.cssText = 'color:var(--muted); font-size:0.85em; margin:-8px 0 14px;';
    sub.textContent = subtitle;
    box.appendChild(sub);

    const items = (statsResult && statsResult.items) || [];
    if (items.length === 0) {
        const empty = document.createElement('p');
    ...
    table.className = 'inn-bookings-table';
    table.innerHTML = `
        <thead><tr><th>#</th><th>Položka</th><th>Prodáno (ks)</th><th>Tržba</th></tr></thead>
        <tbody></tbody>
    `;
    ...
    items.forEach((it, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td>${Number(it.revenue).toLocaleString('cs-CZ')} Kč</td>
        `;
...
async function renderSalesView() {
    ...
    let weekStats, monthStats;
    try {
        [weekStats, monthStats] = await Promise.all([
            fetchSalesStats(7),
            fetchSalesStats(30)
        ]);
    } catch (e) {
        container.innerHTML = '<p style="color:var(--danger);">Nepodařilo se načíst statistiky prodejů.</p>';
        return;
    }
    ...
    note.style.cssText = 'color:var(--muted); font-size:0.85em; margin:-6px 0 18px;';
    note.textContent = 'Zahrnuje jídlo objednané k rozvozu i k rezervaci stolu.';
    container.appendChild(note);

    container.appendChild(renderSalesTable('Tento týden', 'Posledních 7 dní', weekStats));
    container.appendChild(renderSalesTable('Tento měsíc', 'Posledních 30 dní', monthStats));
